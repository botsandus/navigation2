...
| `min_distance_to_path_obstacle` | Path-arc-length horizon (m) within which to scan the upcoming path for obstacles. Starting from the path pose closest to the robot, each subsequent path pose is checked against the local costmap footprint until this accumulated distance is exceeded. If a lethal cell is found, a `NoValidControl` is thrown. Set <= 0.0 to disable. Requires `use_collision_detection` to be enabled. |
