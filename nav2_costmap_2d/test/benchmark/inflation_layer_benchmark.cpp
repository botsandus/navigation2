  ...
  const std::string & path)
{
  ...
  // Overlay differences on base map
  cv::Mat composite = base_map.clone();
  for (unsigned int y = 0; y < size_y; ++y) {
    for (unsigned int x = 0; x < size_x; ++x) {
      ...
      if (val > 0) {
        ...
        if (val < 85) {
          ...
          color = cv::Vec3b(0, 255 - t / 2, 255);  // BGR: reduce green by half
        } else {
          // Orange to red (170-255)
          unsigned char t = static_cast<unsigned char>(((val - 170) / 85.0) * 255);
  ...
  // Flip along horizontal axis (vertical flip)
  cv::Mat composite_flipped;
  cv::flip(composite, composite_flipped, 0);

  return cv::imwrite(path, composite_flipped);
}
