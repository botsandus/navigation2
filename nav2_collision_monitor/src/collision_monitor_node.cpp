  ...
  std::string & state_topic)
{
  ...
  on_set_params_handler_ = node->add_on_set_parameters_callback(
    std::bind(
      &CollisionMonitor::validateExclusionZoneParameters, this, std::placeholders::_1));
  post_set_params_handler_ = node->add_post_set_parameters_callback(
    std::bind(
      &CollisionMonitor::updateExclusionZoneParameters, this, std::placeholders::_1));
...
std::shared_ptr<Source> CollisionMonitor::findSource(const std::string & source_name) const
{
  for (const auto & source : sources_) {
...
rcl_interfaces::msg::SetParametersResult CollisionMonitor::validateExclusionZoneParameters(
...
void CollisionMonitor::updateExclusionZoneParameters(
  const std::vector<rclcpp::Parameter> & parameters)
{
  const std::string suffix = ".exclusion_zones";
  for (const auto & parameter : parameters) {
